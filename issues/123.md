### Issue Title
Corrigir validações HTML em todos os arquivos do repositório

### Problema
A validação de conteúdo está falando em múltiplos arquivos HTML devido a problemas de conformidade com as regras do html-validate:

1. **Botões sem atributo `type`** (`no-implicit-button-type`)
2. **Estilos inline não permitidos** (`no-inline-style`)

Esses problemas foram identificados no job de validação e precisam ser corrigidos em todos os arquivos HTML do repositório.

### Erros identificados no job 61463287992
...
- Job com falha: #6 (commit 39864fff6e7e51d400fa32b67acec54c3dfee744)
...
**Prioridade**: Alta  
**Labels sugeridos**: bug, html, validation, good first issue 
